...
contract Staking {
    ...
    function claimValidtorReward() external zeroAddress returns(bool) {

        address payable staker = msg.sender; // validator address
        
        Validator storage valInfo = validatorInfo[staker];
        uint256 percentageToTransfer = valInfo.amount.div(valInfo.coins).mul(100); // 40 / 100 * 100 => 40%
        uint256 rewardAmount = valInfo.income.mul(percentageToTransfer).div(100); // 10 * 40 / 100 = > 4
        staker.transfer(rewardAmount); // 4
        uint256 remainingDelegatorAmount = valInfo.income.sub(rewardAmount); // 6
        uint valCoins = valInfo.coins; // 100
        uint256 amount = valInfo.amount; // 40 
        distributeRewardToDelegators(remainingDelegatorAmount,valCoins.sub(amount),staker);
        valInfo.income = 0;
