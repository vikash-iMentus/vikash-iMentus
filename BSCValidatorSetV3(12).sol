...
//   using RLPDecode for *;
  ...
  //function deposit(address valAddr) public payable {
   function deposit(address valAddr) external payable onlyCoinbase onlyInit noEmptyDeposit{
       ...
       valInfo.TotalIncome = valInfo.TotalIncome.add(valInfo.income);
...
//   }
  ...
  enum Status {
   ...
   // address[] public highestValidators; // Only Top 21
    ...
    function userProposal() private returns (bytes32[] memory){
      delete userProposalsArray;
      for(uint i=0; i < ProposalsArray.length; i++){
        if(proposals[ProposalsArray[i]].dst == msg.sender){
          userProposalsArray.push(ProposalsArray[i]);
        }
      }
      return userProposalsArray;
    }

    // function userProposalList() public view returns (bytes32[] memory){
    //     userProposal();
    //     return userProposalsArray;
    // }
