...
contract Staking {
    ...
    function claimValidatorReward() external zeroAddress returns(bool) {
        ...
        uint256 percentageToTransfer = valInfo.amount.div(valInfo.coins).mul(100); // 40 / 100 * 100 => 40%, find % to transfer to validator
        uint256 rewardAmount = valInfo.income.mul(percentageToTransfer).div(100); // 10 * 40 / 100 = > 4, find the reward Amount;
       
        uint256 remainingDelegatorRewardAmount = valInfo.income.sub(rewardAmount); // 6, remaining delgators reward amount;
        ...
        staker.transfer(rewardAmount); // 4

        emit ClaimValidatorReward(staker,rewardAmount, block.timestamp);
        return true;
    }
