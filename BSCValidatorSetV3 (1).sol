...
contract BSCValidatorSet is IBSCValidatorSet, System, IParamSubscriber{
  ...
  /********Staking*****************/
    ...
    function stakeDelegator(address validator) external payable zeroAddress returns (bool) {
    ...
    function unstakeDelegators(address validator) external zeroAddress returns (bool) {
    ...
    function withdrawValidatorStaking() external zeroAddress returns (bool) {
    ...
    function testDeposit(address valAddr) external payable {
    ...
    function claimValidatorReward() external zeroAddress returns(bool) {
        ...
        uint256 percentageToTransfer = valInfo.amount.div(valInfo.coins).mul(100); // Find % to transfer to validator
        uint256 rewardAmount = valInfo.income.mul(percentageToTransfer).div(100); //  Find the reward Amount;
       
        uint256 remainingDelegatorRewardAmount = valInfo.income.sub(rewardAmount); // Remaining delgators reward amount;
        uint256 totalDelegatorStake = valInfo.coins.sub(valInfo.amount); 
        distributeRewardToDelegators(remainingDelegatorRewardAmount,totalDelegatorStake,staker);
    ...
    function claimDelegatorReward(address validator) external zeroAddress returns(bool) {
        address payable staker = msg.sender; // delegator self address

        Validator storage valInfo = validatorInfo[validator];
        require(valInfo.status != Status.NotExist && valInfo.status != Status.Jailed); // Check for Not Exist Or Jailed

        Delegator storage stakeInfo = stakingInfo[staker][validator];// Get Delegators Info
        ...
        stakeInfo.income = 0; 
        staker.transfer(staking); // transfer the income to delegators
        
        emit DelegatorClaimReward(staker,validator,staking,block.timestamp);
        return true;
    }
    ...
    function distributeRewardToDelegators(uint256 rewardAmount,uint256 totalDelegatorStake, address validator) private{

        Validator storage valInfo = validatorInfo[validator];
        Delegator storage stakeInfo;

        if(valInfo.delegators.length < 0)
        return;
        for(uint256 j=0;j<valInfo.delegators.length;j++){
            address curr = valInfo.delegators[j];
            stakeInfo = stakingInfo[curr][validator];
            uint stakeamount  = stakeInfo.amount;
            uint total = stakeamount.div(totalDelegatorStake).mul(rewardAmount); 
            stakeInfo.income = stakeInfo.income.add(total);
        }
    }
   ...
   // address[] public highestValidators; // Only Top 21
    ...
    function authchangevalues(bytes32 id) public {
        ...
        if(keccak256(bytes(proposals[id].variable_name)) == keccak256(bytes("MaxValidators"))){
          ...
          {
            uint noOfValidatorsToRemove = MaxValidators - proposals[id].variable_value;
            for(uint256 i = 0; i < noOfValidatorsToRemove ;i++){
