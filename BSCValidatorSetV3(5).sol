...
contract BSCValidatorSet is IBSCValidatorSet, System, IParamSubscriber{
  ...
  // function deposit(address valAddr) external payable onlyCoinbase onlyInit noEmptyDeposit{
       ...
       console.log(valInfo.amount);
       console.log(valInfo.coins);
       ...
       console.log("% to transfer & reward Amount", percentageToTransfer, rewardAmount);
        ...
        console.log("remaining delegator reward amount & tatotal delegator stake", remainingDelegatorRewardAmount, totalDelegatorStake);
       
        distributeRewardToDelegators(remainingDelegatorRewardAmount,totalDelegatorStake,valAddr);
  ...
  enum Status {
    ...
    function distributeRewardToDelegators(uint256 rewardAmount,uint256 totalDelegatorStake, address validator) private{
        ...
        for(uint256 j=0;j<valInfo.delegators.length;j++){
            ...
            uint256 percentageToTransfer = stakeamount.mul(100).div(rewardAmount);
            uint256 rewardDelegatorAmount = rewardAmount.mul(percentageToTransfer).div(100);

            console.log("Check percetage and rewardDelegator amount", percentageToTransfer, rewardDelegatorAmount);
