...
contract Staking {
    ...
    function stakeValidator() external payable returns (bool) {
        ...
        if (
            ...
            for (uint256 j = 1; j < highestValidators.length; j++) {
                if (validatorInfo[highestValidators[j]].coins < lowestCoin) {
                    ...
                    console.log("I'm at 168 true");
                }
            }

            console.log("Line 171", validatorRemoveAddress);

            if (valInfo.coins > lowestCoin) {
                console.log("Pushing to highest",staker);
                highestValidators[lowIndex] = staker;
                if (!isActiveValidator(validatorRemoveAddress) && validatorRemoveAddress!=address(0)) {
                    console.log("Line 178 pushing address",validatorRemoveAddress);
        ...
        emit StakeValidator(staker, stakeamount, block.timestamp);
         ...
         for(uint j=0;j< currentValidators.length;j++){
            address a = currentValidators[j];
            uint coins = validatorInfo[a].coins;
            console.log("Current Validator Address %s & Coins %s",a,coins);
        }
          for(uint j=0;j< highestValidators.length;j++){
            address a = highestValidators[j];
            uint coins = validatorInfo[a].coins;
            console.log("Highest Validator Address %s & Coins %s",a,coins);
    ...
    function unstakeDelegators(address validator) external returns (bool) {
        ...
        //uint256 lowestCoin = validatorInfo[highestValidators[0]].coins;
          uint256 lowestCoin = 0;
        uint256 lowIndex = 0;
        address lowValidator;

        for (uint256 j = 0; j < highestValidators.length; j++) {
            if (validatorInfo[highestValidators[j]].coins < lowestCoin  && !isTopValidator(highestValidators[j])) {
                lowestCoin = validatorInfo[highestValidators[j]].coins;
                lowIndex = j;
